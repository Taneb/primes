...
------------------------------------------------------------------------
-- Prime factorizations and their properties
------------------------------------------------------------------------
...
open import Data.List.Relation.Binary.Permutation.Propositional.Properties.Extra
open import Data.Nat.Base
open import Data.Nat.Divisibility
open import Data.Nat.Divisibility.Extra
open import Data.Nat.Induction
open import Data.Nat.Primality
open import Data.Nat.Primality.Extra
...
factorize (suc (suc n)) = <-rec (λ n′ → ∀ {k} → 2 ≤ n′ → k ≤‴ n′ → k Rough n′ → Factorization n′) factorizeRec (2 + n) {2} (s≤s (s≤s z≤n)) (≤⇒≤‴ (s≤s (s≤s z≤n))) (2-rough-n (2 + n))
  ...
  ... | yes k∣n = record
    ...
    where
      ...
      res = rec (quotient k∣n) q<n {2 + k} 2≤q (≤⇒≤‴ (≮⇒≥ (λ q<k → rough⇒∤ k-rough-n 2≤q q<k (∣-comm k∣n)))) λ {d} d<k d-prime → k-rough-n d<k d-prime ∘ flip ∣-trans (∣-comm k∣n)
...
... | inj₁ p∣a with ∣p⇒≡p∨≡1 p aPrime p∣a
...   | inj₁ refl = as , ↭-refl
